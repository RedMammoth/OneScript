...
Процедура ТестДолжен_ПроверитьАвторизациюПрокси() Экспорт
	...
	Прокси.Установить("http","http://localhost", 3128, "proxy_user", "proxy_pass", Ложь);
	Соединение = Новый HTTPСоединение("http://192.168.0.5", 8081, , , ИнтернетПрокси);
    Ответ = Соединение.Получить(Новый HTTPЗапрос());
    юТест.ПроверитьРавенство(200, Ответ.КодСостояния);
    
    IP = Ответ.ПолучитьТелоКакСтроку();
	юТест.ПроверитьРавенство("192.168.0.9", IP);
